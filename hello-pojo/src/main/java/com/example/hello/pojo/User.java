...
public class User {
    ...
    @ApiModelProperty(position = 2, value = "用户名", example = "admin")
    private String username;

    @ApiModelProperty(position = 3, value = "密码")
    private String password;

    @ApiModelProperty(position = 4, value = "密码盐")
    private String passwordSalt;

    @ApiModelProperty(position = 5, value = "姓名", example = "超级管理员")
    private String name;

    @ApiModelProperty(position = 6, value = "性别：0-未知，1-男，2-女", example = "0")
    private Integer sex;

    //@JsonSerialize(using = WriteDateAsSecondsSerializer.class)
    //@JsonDeserialize(using = WriteDateAsSecondsDeserializer.class)
    @ApiModelProperty(position = 7, value = "出生日期", example = "1609862400")
