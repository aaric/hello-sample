...
public class FileController {
    ...
    public String convert(@RequestBody Converter body,
        ...
        try {
            if (fileUtility.getConvertExts().contains(fileExt)) {
                ...
                Path path = FileSystems.getDefault().getPath(storagePathBuilder.getFileLocation(correctedName));
    ...
    public String create(@RequestParam("fileExt") String fileExt,
        ...
        if (fileExt != null) {
            try {
                ...
                return "redirect:editor?fileName=" + URLEncoder.encode(fileName, StandardCharsets.UTF_8.name());
            } catch (Exception ex) {
    ...
    public ResponseEntity<Resource> assets(@RequestParam("name") String name) {
        Path path = FileSystems.getDefault().getPath("assets", "sample", fileUtility.getFileName(name));
    ...
    public ResponseEntity<Resource> csv() {
        Path path = FileSystems.getDefault().getPath("assets", "sample", "csv.csv");
        String fileName = path.toString();
        return downloadFile(fileName);
    }
